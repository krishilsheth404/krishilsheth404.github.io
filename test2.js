...
app.post('/result', async(req, res) => {
    ...
    // const urlForSS = `https://in.search.yahoo.com/search;_ylt=?p=site:onebharatpharmacy.com+${nameOfMed}`;
        ...
        await Promise.all(item.map(async(item) => {
            if (item != undefined) {
                if (item.includes('netmeds')) {
                    ...
                    final.push(await extractDataOfTata("https://www.1mg.com/search/all?name=vicks"))
                        // await fetchItem(item) 
                } else if (item.includes('pharmeasy')) {
